...
def generate_experiment_report(
    ...
    md_lines = [
        "# Modern NLP Classification Experiment Report",
        "",
        "## 1. Configuration",
        "```json",
    ...
    if benchmark_comparison and "baseline" in benchmark_comparison:
        ...
        b_enc = base_perf.get("inference_time_sec", 0)
        f_enc = fine_perf.get("inference_time_sec", 0)
        md_lines.append(f"| Inference Time (s) | {b_enc:.3f} | {f_enc:.3f} | {f_enc - b_enc:+.3f} |")
    ...
    html_lines = [
        ...
        "</head>",
        "<body>",
        "<h1>Modern NLP Classification Experiment Report</h1>",
    ...
    if benchmark_comparison and "baseline" in benchmark_comparison:
        ...
        html_lines.append(f"<tr><td>Inference Time (s)</td><td>{b_enc:.3f}</td><td>{f_enc:.3f}</td><td>{f_enc - b_enc:+.3f}</td></tr>")
