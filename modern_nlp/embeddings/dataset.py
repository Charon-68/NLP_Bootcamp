...
@DatasetRegistry.register("EmbeddingDataset")
class EmbeddingDataset(BaseDataset):
    ...
    @classmethod
    def validate(cls, dataset: Dataset) -> ValidationReport:
    ...
    @classmethod
    def preprocess(cls, dataset: Dataset, split_name: str = "train", cache_dir: Optional[str] = None) -> Dataset:
        ...
        validation_report = cls.validate(dataset)
...
# Keep backward compatibility for pipeline.py
def load_dataset(seed: int = 42) -> Tuple[Dataset, Dataset]:
    return EmbeddingDataset.load(seed=seed)

def validate_dataset(dataset: Dataset) -> ValidationReport:
    return EmbeddingDataset.validate(dataset)

def prepare_dataset(dataset: Dataset, split_name: str = "train", cache_dir: Optional[str] = None) -> Dataset:
    return EmbeddingDataset.preprocess(dataset, split_name=split_name, cache_dir=cache_dir)
